...
from omnimalloc.primitives.queries import get_per_allocation_pressure, get_pressure
...
@pytest.mark.parametrize("pattern", SYNC_PATTERNS)
def test_sync_patterns_queries_agree_on_small_instances(pattern: str) -> None:
    source = SyncPatternSource(
        num_allocations=10, num_threads=3, pattern=pattern, seed=5
    )
    allocations = source.get_allocations()
    peaks = get_per_allocation_pressure(allocations)
    assert max(peaks.values()) <= get_pressure(allocations)
    assert all(peaks[a.id] >= a.size for a in allocations)
