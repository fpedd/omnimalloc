...
from omnimalloc.primitives.queries import Guarantee, get_pressure
...
def test_concurrent_tiling_pressure_equals_capacity(num_syncs: int) -> None:
    capacity = 1024 * 1024
    source = ConcurrentTilingSource(
        num_allocations=64, num_threads=4, num_syncs=num_syncs, capacity=capacity
    )
    assert get_pressure(source.get_allocations(), Guarantee.ANTICHAIN) == capacity
