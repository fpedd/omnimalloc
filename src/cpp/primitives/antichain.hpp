...
// scale, not for the 10k+ hot path. A finite `work_budget` bounds both the
// linearize attempt and the flow construction (dominance-counting work,
// O(k * m * d) over deduplicated clock rows); the flow path throws instead
